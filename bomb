...
pause() {
    read -n1 -r -p "Press Any key 2 Continue..." key
}
banner() {
    clear
    echo -e "\e[1;31m"
    if ! [ -x "$(command -v figlet)" ]; then
        echo 'Introducing Bomb'
    else
        figlet Bomb
    fi
    if ! [ -x "$(command -v toilet)" ]; then
        echo -e "\e[4;34m This Bomber Was Created By Godcy"
    else
        echo -e "\e[1;34mCreated By \e[1;34m"
        toilet -f mono12 -F border Nitro
    fi
    echo -e "\e[1;34m For Any Queries Join Me!!!\e[0m"
    echo -e "\e[1;32m           Telegram: "
    echo -e "\e[4;32m   YouTube: "
    echo " "
...
if [ -f .update ];then
    ...
    echo .
    install_deps
    echo This Script Was Made By Godcy > .update
